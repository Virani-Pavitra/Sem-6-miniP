...
const MessageContainer = () => {
    ...
    const renderMessages = () => {
        ...
        return selectedChatMessages.map((message, index) => {
            const messageDate = moment(message.timestamp).format("YYY-MM-DDD");
                    ...
                    message.sender._id !== userInfo.id ? (
                        <div className="flex items-center justify-start gap-3">
                            <Avatar className="h-8 w-8 rounded-full overflow-hidden">
                                ...
                                <AvatarFallback
                                    className={`uppercase h-8 w-8 text-lg flex items-center 
                                justify-center rounded-full ${getColor(message.sender.color)}`}
                                >
                            ...
                            <span className="text-xs text-white/60"> {moment(moment.timestamp).format("LT")} </span>
                        </div>) : (
                        <div className="text-xs text-white/60 mt-1"> {moment(moment.timestamp).format("LT")} </div>
                    )
                }
            </div>
        )
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hidden p-4 px-8 md:w-[65vw] lg:w-[70vw] xl:w-[80vw] w-full">
            ...
            <div ref={scrollRef} />
            {
                showImage && <div className="fixed z-[1000] top-0 left-0 h-[100vh] w-[100vw] flex items-center justify-center backdrop-blur-lg flex-col">
                    <div>
                        <img src={`${HOST}/${imageUrl}`} className="h-[80%] w-[95%] bg-cover top-10" />
                    </div>
